...
function main()
    ...
    # Normalize is not strictly necessary, but good practice
    normalize!(x)
    normalize!(y)
    ...
    #outputtting convolutions to different files for plotting in external code
    # note: we are outputting just the real component beacause the imaginary
    # component is virtually 0
    open("fft.dat", "w") do io
        writedlm("./fft.dat", real(fft_output))
    end
    plot(101:300, real(fft_output))
end
main()